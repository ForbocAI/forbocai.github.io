...
export const Servitor = () => {
    ...
    <section id="servitor" class="chapter chapter-day servitor">
        <div class="container">
            ...
            <div class="servitor-pair">
                <section class="servitor-does">
                    ...
                    <ul>
                        ...
                        <li><strong>Weighs your values, not its own.</strong> It reasons from the moral frame your world supplies — a paladin's code, a guild's rules, a corrupt institution. Supply none and she reasons only from what she wants and what she has seen, which reads thinner, because the frame is the part worth writing.</li>
            ...
            <p class="servitor-moat">We hold no player data and no world state, so nothing compounds for us in a database — that is deliberate, and it means the first question your legal team asks has a one-line answer. What compounds is the bench: every world a release is qualified against stays in it, and the bar it cleared becomes the floor for the next one. Today that bench is entirely ours — three playable builds and the worlds behind the harness that qualifies each release. That is small, and it is exactly what this round buys: a pilot leaves behind a qualification world — scenarios, rules and edge cases agreed in writing — never its runtime data and never its players'. And a bench world is not a test suite we could have written for ourselves. It is a real studio's canon, with the rules that world actually enforces and the places its writers know it strains. You cannot commission that. You can only be let in. That term is the hardest thing we ask a pilot for, and we have not closed one yet. What a studio gets for it is the floor itself — the bar a pilot raises is the bar every later studio inherits, and the pilot is the first to stand on it. We are asking for the rules of a world, in writing. Never the people in it. A competitor starts that at zero no matter whose API they rent, and the first thing they have to do is persuade a studio to let them inside its canon.</p>

            <p class="servitor-moat">Every other layer of an NPC stack is a quarter's work for a competent team. A model that composes into sixteen minds, holds a world it was never trained on, and refuses to invent its way out of a gap is not, and headcount buys less of it than you would expect, because the work is sequential: each release is qualified against the bar the last one cleared, and the bar is built from worlds a studio had to agree to let us inside. A larger team reaches the first gate sooner. It does not get to skip the gate. That is the part of ForbocAI that compounds, and it is the part a competitor cannot reach by wiring up somebody else's API.</p>
        </div>
    </section>
    `;
};
