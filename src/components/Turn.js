...
export const Turn = () => {
  ...
  constraints: ['The letter does not open on a debt. Only on news of Aldren.'],
                ...
                <p class="turn-note"><strong>Your narrative lead reads a thousand of her reasons before you ship.</strong> Every turn is a payload — the document, the memory, the world, the legal actions — so the whole cast can be run against a corpus of them on a schedule, and the reasons read in bulk. It is the table read your team already does, at a scale no table read has ever reached, with the reason on the page instead of the line. They approve the document and they approve the reasons. When a reason is wrong it traces to a document, the document is in their repo, and the fix is a Tuesday.</p>
                    ...
                    <pre class="turn-code"><code>// legalActions: ['offer_item', 'refuse'] — the same two, again
{
  ...
  action: { type: 'offer_item', payload: { item: 'corner_room_key' } },
                    ...
                    <p class="turn-note"><strong>And there is a second fault, in the exhibit we are proudest of.</strong> Nothing in that call established that the corner room exists, is empty, or is hers to give. She reasons from the evidence you send, and evidence you do not send is evidence she can reach past. Your validation rules refuse the key or they do not — that is clause twelve again, on the block we like best.</p>
            ...
            <p class="turn-consequence"><strong>And the lock has nothing to hold her against.</strong> Her judgment was never a string, so it was never on the string table, and the lock that cut your arc has nothing to hold her against. That is the freedom and it is also the problem. She reaches the decision once and a decision is language-independent — but delivery is not: register, idiom, honorifics, and what a refusal sounds like in a language where refusing is done sideways. Shipping her in nine languages means trusting her to be the same person in all nine and native in each. That is clause twenty-five, the hardest line in the Standard.</p>

            <p class="turn-consequence"><strong>We do not make voices, and we are not going to.</strong> Servitor™ returns a judgment and the words that carry it. No performer's voice is trained, cloned or generated here. A line nobody wrote is a line nobody recorded, so she starts where a cast is largest and least served — the hundreds your budget was always going to subtitle, who have been reading the same six barks since launch. Your performed principals stay performed, exactly as written and recorded. Where a studio does want her reaching voice, that is a contract between the studio and the performer, and our position is the one we would want if we were on the other side of it: a voice is a person's work, it is used with their consent, and it is paid for.</p>
