...
export const Turn = () => {
  ...
  metadata: { valid: true },
                    ...
                    <p class="turn-note"><strong>A lore error is the second, and there is one in the turn above.</strong> Look at <em>He went north.</em> She refused the letter, and the action check guarantees that refusal — your state is safe and your inventory never moved. But the road she gave is prose, and nothing in this turn checked it against your world. If act two moved her brother south, she has just told your player otherwise, and it is in their quest log by morning. That is gate twelve. We would rather print it on our own demo than name it three paragraphs away from one.</p>
                    ...
                    <p class="turn-note">Two more faults belong here rather than in a list you have to go and find. A reply can promise what your rules then refuse — she says the letter is yours, your policy rejects the handover, and the player already heard the promise. That is gate eighteen, and it is the most common shipped failure in this category. And on a valid turn the memory write is not checked against your rules the way an action is, so the one artifact that persists and shapes every later judgment goes in unvalidated: gate twenty. Neither is a fix. They are the seams you hold while we owe you three.</p>
                    <p class="turn-note">The same plainness about time and cost, because they are the same fact. One character judgment is one call: the dialogue, the reasoning behind it and the structured action come back from the same attempt—not a chain of calls per line, not a re-roll to get a parseable action, not a second request to explain the first. This turn crossed a network and ran under a hard deadline. If it cannot finish in that budget and meet its rules, the turn comes back marked invalid and authorizes nothing—no action, no memory write, no change to your state—and the line the player hears in that moment is yours to author. Be clear about who holds that stopwatch: today it is ours, one server-side budget for every scene, not a dial you turn per scene — gate nineteen. How often a turn misses it is gate seventeen, measured and published when it is measured.</p>
                </div>
            </div>
