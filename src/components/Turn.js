...
export const Turn = () => {
  ...
  metadata: { valid: true },
                    ...
                    <p class="turn-note">The reply is checked too, and it is worth being exact about what that means. It must name the action she actually took, must not argue for the one she refused, and must not claim a comparison the supplied evidence does not carry. Two different faults get called the same thing, and only one of them is ours to claim. <strong>The first is the fault this company exists to remove.</strong> The model holds no self to leak into her, so there is nothing inside it that wants to be agreeable, or wry, or itself, at your character's expense. That is the invention. The measurement that turns it into a public number—persona-hold under adversarial play, against a named alternative, on a cast we did not author—is gate thirteen, and it is next. <strong>She can still get a fact about your world wrong.</strong> Her prose is not checked against your lore; the action boundary is what protects your state. That second one is the twelfth gate, and it is on the list precisely so it is owed to you rather than disclaimed.</p>
